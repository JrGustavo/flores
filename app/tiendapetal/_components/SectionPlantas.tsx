...
export function SectionPlantas() {
    const [showAddressModal, setShowAddressModal] = useState(false);
    const [visibleProducts, setVisibleProducts] = useState(
        products.slice(0, 5) // Limita los productos visibles inicialmente
    );
    ...
    const scroll = (direction: "left" | "right") => {
        if (carouselRef.current) {
            ...
            if (direction === "left") {
                newStartIndex = Math.max(0, currentStartIndex - ITEMS_PER_PAGE);
            } else {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-navy-900">Plantas</h2>
            ...
            >
                {visibleProducts.map((product) => (
                    <div key={product.id} className="flex-none w-[200px] snap-start">
                        <div className="relative bg-white rounded-lg p-4 h-full shadow-md">
                            {product.discount > 0 && (
